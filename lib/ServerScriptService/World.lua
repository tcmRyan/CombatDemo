...
local ItemDB = require(ReplicatedStorage.ItemDB)
...
function World:AddItem(itemId, count)
    ...
    -- No the item does not exist
    -- Add it as a new item
    table.insert(self.mItems, {id = itemId, count = count})
end
...
function World:RemoveItem(itemId, amount)
    ...
    assert(false) -- never should hit this line
end
